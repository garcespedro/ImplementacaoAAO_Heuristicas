...
import Heuristicas.HeuristicasPesquisaLocal.Procedimento2Opt;
...
public class Procedimento2OptVizinhoMaisProximo {
    public static void main(String[] args) {
        ...
        for (int i = 1; i < cidadesArray.length; i++) {
            ...
            for (int j = 0; j < cidadesArray.length; j++) {
                if (!visitadas[j]) {
                    double dist = atual.distanciaPara(cidadesArray[j]); // <== CORRIGIDO AQUI
                    if (dist < menor) {
                        menor = dist;
                        proxima = j;
                    }
        ...
        Procedimento2Opt opt = new Procedimento2Opt(grafoCompleto, cidadesList);

        long inicio2Opt = System.currentTimeMillis();
        double distInicial = opt.calcularDistanciaTotal(percurso);
        List<Integer> percursoOtimizado = opt.resolver2Opt(percurso);
        double distFinal = opt.calcularDistanciaTotal(percursoOtimizado);
        long fim2Opt = System.currentTimeMillis();

        // Resultados
        System.out.println("Distância inicial (Vizinho + ciclo): " + distInicial);
        System.out.println("Distância após 2-opt: " + distFinal);
        ...
        System.out.println("Tempo de execução 2-opt: " + (fim2Opt - inicio2Opt) + " ms");
    }
}